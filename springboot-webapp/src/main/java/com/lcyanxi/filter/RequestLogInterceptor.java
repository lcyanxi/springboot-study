...
public class RequestLogInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (request.getMethod().equals("POST")){
            try{
                ...
                RequestLogger.setAttribute("param", requestWrapper.getBody());
            }catch (Exception e){
                log.error("requestLogInterceptor post request is exception url:[{}]",RequestLogger.formatRequestLog(),e);
            }
        }else {
    ...
    public void afterCompletion(
        ...
        // add exception information into request log
        // when exception is handled by ExceptionHandler, the passed ex is null
        // get original exception from request
        Exception exception = ex != null ?
                ex : (Exception) request.getAttribute(DispatcherServlet.EXCEPTION_ATTRIBUTE);
        if (exception != null) {
            // runtime error, use a 500 error code
